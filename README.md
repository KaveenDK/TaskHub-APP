...
Students often ask: _"If React Native is cross-platform, why do I need to install specific Android and iOS tools?"_
...
2.  **SDKs are the Build Tools (The Factory):**
    ...
    - **Xcode (iOS SDK):** Is required to take that _same_ JavaScript code and compile it into an iOS App.

**In short:** The _development_ requires platform-specific tools (SDKs) to do the heavy lifting of compiling, even though your _coding_ is done in a cross-platform language.
...
- **BaaS (Backend as a Service):** Cloud-based backend services that provide developers with APIs for backend functions, allowing them to focus entirely on the Frontend.
  - _Examples:_ Firebase, Supabase, AWS Amplify.
...
2.  **Frontend + Custom Backend + Firebase:**
    - A hybrid approach.
    - _Example:_ Using a Node.js server for complex business logic while offloading Authentication and Notifications to Firebase.
...
- **Use Case:** Storing **public** or **non-critical** data.
  - _Examples:_ App theme (Dark/Light), user preferences, local caching.
...
- **Use Case:** Storing **private** or **critical** data.
  - _Examples:_ User Authentication Tokens (JWT), API Keys, Passwords.
